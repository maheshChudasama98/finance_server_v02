...
module.exports.emailList = [
	...
	},
	{
		...
		Subject: "🚨 Server Down Detected",
		Content: `
		<h2 style="color:#ff4d4f;">⚠️ Server Down Notification</h2>
		<p><strong>Time:</strong> {__DownTime__}</p>
		<p><strong>Project:</strong> {__ProjectName__}</p>
		<p><strong>Port:</strong> {__ServerPort__}</p>
		<p><strong>Database:</strong> {__DatabaseName__}</p>
		<p><strong>URL:</strong> <a href="{__DefaultUrl__}" target="_blank">{__DefaultUrl__}</a></p>
		<p><strong>Status Code:</strong> {__StatusCode__}</p>
		<p><strong>Error Details:</strong> {__ErrorMessage__}</p>
		<hr/>
		<p>This alert was automatically generated by your monitoring system.</p>
		<p style="color:#888;font-size:13px;">Please check the server logs or restart the service if necessary.</p>
	`,
		Slug: "server_down",
	},
];
