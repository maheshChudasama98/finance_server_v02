...
exports.TransactionFetchListController = async (payloadUser, payloadBody) => {
    try {
        ...
        if (FilterBy?.PartyFullName) {
            ...
            whereCondition[Op.or] = [
                { '$fn_party.PartyFirstName$': { [Op.like]: "%" + FilterBy?.PartyFullName + "%" } },
                { '$fn_party.PartyLastName$': { [Op.like]: "%" + FilterBy?.PartyFullName + "%" } },
                { '$fn_party.Email$': { [Op.like]: "%" + FilterBy?.PartyFullName + "%" } },
                fullNameCondition
            ];
        };
