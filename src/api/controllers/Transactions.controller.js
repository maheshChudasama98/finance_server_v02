...
exports.TransactionFetchListController = async (payloadUser, payloadBody) => {
  try {
    ...
    if (FilterBy?.PartyFullName) {
      const fullNameCondition = Sequelize.literal(
        `CONCAT(fn_party.PartyFirstName,' ', fn_party.PartyLastName) LIKE '%${FilterBy?.PartyFullName}%'`
      );
    ...
    const fetchList = await TransactionsModel.findAll({
      attributes: [
        ...
        [
          Sequelize.col("fn_sub_category.SubCategoriesName"),
          "SubCategoriesName",
        ],
        [
          Sequelize.literal(`(
  ...
  try {
    ...
    const partyList = await PartiesModel.findAll({
      attributes: [
        ...
        "updatedAt",
        [
          Sequelize.literal("CONCAT(PartyFirstName, ' ', PartyLastName)"),
          "FullName",
        ],
      ],
      where: whereCondition,
