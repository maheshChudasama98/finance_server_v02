...
async function createTransactionStatement() {
	...
	for (let index = 0; index < UserList.length; index++) {
		...
		if (fetchList.length > 0) {
			...
			let fileName = `${moment(EndDate).subtract(0, "months").format("MMM_YYYY")}.pdf`;
...
async function createStatementPDF(OrgId, BranchId, UserId, Month) {
	try {
		const {StartDate, EndDate} = await durationFindFun("This_Month", Month);
		...
		const mailValue = await emailHelper(emailContent, findMail.Subject, Title, element?.Email, [
			{
				filename: fileName,
				path: pdfPath,
			},
...
module.exports = {createTransactionStatement, createStatementPDF};
